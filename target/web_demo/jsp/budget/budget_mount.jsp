...
<script>
    ...
    layui.use('table', function(){
        var tableGuidance = layui.table //Guidance表格
            ,tableBudget = layui.table //Budget表格
            ,tableCostLayer = layui.table; //CostLayer表格
        ...
        tableGuidance.render({
            ...
            ,where: { field: urlField, value: urlId} //请求后端接口的条件，该处就是条件错误点，按照官方给出的代码示例，原先写成了 where: { key : { type: "all" } }，结果并不是我想的那样，如此写，key 将是后端的一个类作为参数，里面有 type 属性，如果误以为 key 是 Layui 提供的格式，那就大错特错了
            ,totalRow: true //开启合计行
            ,url: '${pageContext.request.contextPath}/budget/mountGuidanceBody' // 使用url加载（可与data加载同时存在）
            // ,data:data //数据接口
            ,cellMinWidth: 80
            ,event: true
            ,cols: [[{type:'numbers', title:'序号', fixed: true}
        ...
        tableCostLayer.render({
            ...
            ,where: { field: urlField, value: urlId} //请求后端接口的条件，该处就是条件错误点，按照官方给出的代码示例，原先写成了 where: { key : { type: "all" } }，结果并不是我想的那样，如此写，key 将是后端的一个类作为参数，里面有 type 属性，如果误以为 key 是 Layui 提供的格式，那就大错特错了
            ,totalRow: true //开启合计行
            ,url: '${pageContext.request.contextPath}/budget/mountCostLayerBody' // 使用url加载（可与data加载同时存在）
            ,cellMinWidth: 100
            ,event: true
            ,cols: [[{type:'numbers', title:'序号', fixed: true}
